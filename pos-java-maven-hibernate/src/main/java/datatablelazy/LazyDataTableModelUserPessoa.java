...
public class LazyDataTableModelUserPessoa<T> extends LazyDataModel<UsuarioPessoa> {
	...
	@Override // metodo para organiza a quantidade de registro para cada pagina
	public List<UsuarioPessoa> load(int first, int pageSize, String sortField, SortOrder sortOrder,
			Map<String, Object> filters) {
		list = dao.getEntityManager().createQuery(sql).setFirstResult(first).setMaxResults(pageSize).getResultList();
